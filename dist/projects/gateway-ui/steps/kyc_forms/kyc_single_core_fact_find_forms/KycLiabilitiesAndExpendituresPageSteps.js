...
const KycLiabilitiesAndExpendituresPageLocators_1 = require("@pages/kycElementLocators/kyc_core_fact_find_locators/KycLiabilitiesAndExpendituresPageLocators");
...
class KycLiabilitiesAndExpendituresPageSteps extends BaseKYCSteps_1.BaseKYCSteps {
    ...
    async completeKYC_LiabilitiesAndExpenditures() {
        ...
        await this.assert.assertHeadingVisible('Liabilities & Expenditures', 15000);
    ...
    async answerHasMortgageOnProperty(answer) {
        ...
        (0, src_1.expect)(selected, 'Should have selected an answer for mortgage question').toBeDefined();
        (0, src_1.expect)(selected, 'Selected answer should not be empty').not.toBe('');
    ...
    async selectCurrentMortgageEndTerm(answer) {
        ...
        if (!(await this.action.ensureVisibleOrSkip(q, question)))
            return;
        await (0, src_1.expect)(q).toBeVisible();
    ...
    async selectMortgageLender(value) {
        ...
        await (0, src_1.expect)(this.page.getByText(chosen, { exact: false }).first()).toBeVisible();
    ...
    async fillFixedLengthYears(value) {
        ...
        await (0, src_1.expect)(input).toHaveValue(value);
    }
    ...
    async setProductStartDate(minYearsAgo, maxYearsAgo) {
        ...
        await (0, src_1.expect)(input).toHaveValue(date);
        this.logInfo(`✓ ${label} set: ${date}`);
        return date;
    }
    async answerOtherLiabilities(answer) {
        const question = 'Do you have any other liabilities?';
        await (0, src_1.expect)(this.page.getByText(question, { exact: false }).first()).toBeVisible();
        await this.action.setRadioByQuestion(question, answer);
        this.logInfo(`✓ ${question}: ${answer}`);
    }
    /* =========================
  ...
  ========================= */
    ...
    async assertTotalLiabilitiesCalculatedCorrectly() {
        ...
        // -------------------------------------------------
        await (0, src_1.expect)(this.locators.totalLiabilitiesBalance).toBeDisabled();
        await (0, src_1.expect)(this.locators.totalLiabilitiesMonthly).toBeDisabled();
        await (0, src_1.expect)(this.locators.totalLiabilitiesYearly).toBeDisabled();
        // -------------------------------------------------
    ...
    async fillCommittedExpenditures() {
        const items = [
            ...
            { name: 'Ground rent and service charge', monthly: src_1.TestDataGenerator.randomNumber() },
            {
    ...
    async committedBaseKeyByName(name) {
        ...
        await (0, src_1.expect)(control, `Committed expenditure name not found: "${name}"`).toHaveCount(1);
    ...
    async fillMonthlyAndAssertYearly(name, monthly) {
        ...
        await (0, src_1.expect)(monthlyInput, `Monthly input not found for "${name}"`).toHaveCount(1);
        await (0, src_1.expect)(yearlyInput, `Yearly input not found for "${name}"`).toHaveCount(1);
    ...
    async assertTotals(expectedMonthlyTotal) {
        ...
        await (0, src_1.expect)(monthlyTotal).toBeDisabled();
        await (0, src_1.expect)(yearlyTotal).toBeDisabled();
