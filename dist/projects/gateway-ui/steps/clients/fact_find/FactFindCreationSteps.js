...
class FactFindCreationSteps extends BasePage_1.BasePage {
    ...
    async waitForFactFindHistoryTable() {
        await (0, test_1.expect)(this.factFindLocators.factFindHistoryTable).toBeVisible({ timeout: 60000 });
    }
    /**
     * Creates a new Fact Find and launches the KYC app.
     * Returns the page that contains KYC (either a new tab or the same tab).
    ...
    async assertCreateDateFormat(createDateText) {
        if (!TextHelper_1.TextHelper.isGatewayDateTime(createDateText)) {
            throw new Error(`Create Date format invalid. Expected "2 March 2026 at 20:30" but got: "${createDateText}"`);
        }
    }
    async assertCreateDateWithinTolerance(createDateText, createClickedAt) {
...
FactFindCreationSteps.CREATE_DATE_TOLERANCE_MS = 60000; // 60 seconds
